...
if(isset($_POST['download'])){
    for($i = 0; $i < count($_SESSION["files"]); $i++){
        array_push($_SESSION["durations"], $_POST["time$i"]);
        array_push($_SESSION["position"], $_POST["position$i"]);
        echo $_SESSION["durations"][$i] . "<br/>";
        echo $_SESSION["files"][$i] . "<br/>";
        echo $_SESSION["position"][$i] . "<br/>";
        echo "<br/>";
    }

    $gc = new GifCreator();
    $gc->create(reorderFiles(), $_SESSION["durations"], 10);
    $gifBinary = $gc->getGif();
    file_put_contents("uploads/".$_SESSION["folder"]."/gif.gif", $gifBinary);

    header("Cache-Control: public");
    header("Content-Description: File Transfer");
    header("Content-Disposition: attachment; filename=gif.gif");
    header('Content-type: image/gif');
    header("Content-Transfer-Emcoding: binary");
    readfile("uploads/".$_SESSION["folder"]."/gif.gif");
    header("Location: index.php?dowloadsuccess");
}

//delete files
for($i = 0; $i < count($_SESSION["files"]); $i++){
    if(isset($_POST["delete$i"])){
        array_splice($_SESSION["files"], $i, 1);
        array_splice($_SESSION["thumbnails"], $i, 1);
        header("Location: index.php?dowloadsuccess");
    }
}
...
function reorderFiles(){
    $newOrder = array();
    for($i = 0; $i < count($_SESSION["files"]); $i++){
        for($j = 0; $j < count($_SESSION["position"]); $j++){
            if($_SESSION["position"][$j] == $i+1){
                array_push($newOrder, $_SESSION["files"][$j]);
            }
        }
    }
    return $newOrder;
